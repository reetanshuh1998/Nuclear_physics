...
cases = [
    (np.radians(1.0), np.radians(1.0), "theta_n = theta_b = 1 deg"),
    (np.radians(1.0), np.radians(10.0), "theta_n = 1 deg, theta_b = 10 deg"),
    (np.radians(10.0), np.radians(1.0), "theta_n = 10 deg, theta_b = 1 deg"),
    (np.radians(10.0), np.radians(10.0), "theta_n = theta_b = 10 deg")
]

Ec = 40.0
for th_n, th_b, label in cases:
    kin = reaction_au.solve_final_state_from_Ec(Ec, th_b, 0.0, th_n, np.pi)
    if kin is None:
        continue
    
    # Calculate lab momenta:
    # Projectile: p_a_lab = reaction_au.p_a_lab * np.array([0, 0, 1])
    # Core: p_b_lab
    p_b_mag = kin['p_b_mag']
    p_b_lab = p_b_mag * np.array([np.sin(th_b), 0.0, np.cos(th_b)])
    
    # Neutron: p_c_lab
    p_c_mag = kin['p_c_mag']
    p_c_lab = p_c_mag * np.array([-np.sin(th_n), 0.0, np.cos(th_n)])
    
    # Target: p_t
    p_t = kin['p_t']
    
    # Projectile lab:
    p_a_lab = reaction_au.p_a_lab * np.array([0.0, 0.0, 1.0])
    
    # Calculate Jacobi wavevectors:
    # q_a = relative momentum in initial channel / hbar
    q_a = (reaction_au.m_t * p_a_lab) / (reaction_au.m_a + reaction_au.m_t) / HBAR_C
    
    # q_b = relative momentum of b and t in final channel / hbar
    q_b = (reaction_au.m_t * p_b_lab - reaction_au.m_b * p_t) / (reaction_au.m_b + reaction_au.m_t) / HBAR_C
    
    # q_c = relative momentum of c and b-t in final channel / hbar
    q_c = ((reaction_au.m_b + reaction_au.m_t) * p_c_lab - reaction_au.m_c * (p_b_lab + p_t)) / (reaction_au.m_a + reaction_au.m_t) / HBAR_C
    ...
    d3sigma_mb = d3sigma * 10.0 * 0.82
    
    print(f"=== {label} ===")
    print(f"  k2 = {k2:.4f}, xi_0 = {xi_0:.4f}")
    print(f"  I_abs = {np.abs(I):.4e}")
    print(f"  dsig_mb = {d3sigma_mb:.4f} mb/MeV.sr")
